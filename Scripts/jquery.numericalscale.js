      ...
      return this.each(function () {
        ...
        function addScoreToPage(score) {
          ...
          if (replace(scores, score.question, score.value)) {
            scoreUl = document.getElementById('scores-' + $list.attr('id') + '-ul');
        ...
        function createQuestion(oList, oItem, index) {
          ...
          question = "<div class='opinion-question'>" + oItem.text() + "</div>" + "<div class='opinion-responses'>" + "<span class='bipolar-adjective'>" + defaults.lowOpinionAnswer + "</span>";
          // Create a radio button group for each question.
          for (i = 1; i <= defaults.responseRange; i += 1) {
            question += "<span class='response-choice'><input type='radio' name='" + qName + "' value='" + i + "' class='radio'";
        ...
        }).filter(':odd').addClass('alt'); // End of function for each child of target element.

        // Set up actions based on the disciplines. 
        $list.wrap('<div id="wrap-' + $list.attr('id') + '" class="survey-wrapper"></div>');
        $list.after('<div id="scores-' + $list.attr('id') + '" class="scores"></div>');
        $list.after('<input type="button" id="submitBtn" class="button btnStyle" value="Show My Gifts" />'); // disabled="disabled" />');
        ...
        $('input[type="radio"]').change(function (e) {
          ...
          discipline = $(e.target).closest('li[class~="question"]').attr('data-discipline');
        ...
        function submitSurvey() {
          var surveyId, dNumber, dWidth, maxHeight, barHeight, tallBarHeight, i, ii, dScore, discipline;
